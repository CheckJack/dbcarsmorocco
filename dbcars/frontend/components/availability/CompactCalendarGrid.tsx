...
function getDaysForMonthGrid(year: number, month: number) {
  ...
  // Only add actual days from the current month
...
export default function CompactCalendarGrid({
  ...
  // Generate day headers starting from the first day of the month
  const dayHeaders = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  const visibleDayHeaders = dayHeaders.slice(startDayOfWeek).concat(dayHeaders.slice(0, startDayOfWeek));
    ...
    <div className="bg-gray-900 p-6 pb-8">
      ...
      <div className="grid grid-cols-7 gap-2 mb-3">
        {visibleDayHeaders.map((day) => (
          <div
            key={day}
            className="text-center text-sm font-bold text-gray-300 py-3 bg-gray-800 rounded-xl"
          >
            {day}
          </div>
        ))}
      ...
      {/* Calendar grid - starts at the correct day of week */}
      ...
      {/* Legend */}
      <div className="mt-6 flex flex-wrap gap-4 text-sm border-t border-gray-800 pt-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-gray-300">Reserved</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-blue-100 border-2 border-blue-300 rounded"></div>
          <span className="font-medium text-gray-300">Out/On Rent</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-purple-100 border-2 border-purple-300 rounded"></div>
          <span className="font-medium text-gray-300">Returned</span>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-gray-300">Blocked</span>
        </div>
      </div>
    </div>
  );
