...
export default function AdminCouponsPage() {
  ...
  const getUsageDisplay = (usageCount: number, usageLimit: number | null) => {
      ...
      return (
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-2">
            <Users className="w-4 xl:w-5 h-4 xl:h-5 text-gray-400" />
            <span className="text-sm xl:text-base font-bold text-white">{usageCount}/{usageLimit}</span>
          </div>
    ...
    return (
      <div className="flex items-center gap-2">
        <Users className="w-4 xl:w-5 h-4 xl:h-5 text-gray-400" />
        <span className="text-sm xl:text-base font-medium text-gray-300">{usageCount} <span className="text-gray-500">(unlimited)</span></span>
      </div>
    );
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2">Coupons Management</h1>
          <p className="text-gray-400 text-sm sm:text-base md:text-lg">Create and manage discount coupons for customers</p>
        </div>
        <button
          onClick={() => {
            setShowForm(true);
      ...
      {loading ? (
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full divide-y divide-gray-800 hidden lg:table">
              <thead className="bg-gray-800/50">
                <tr>
                  <th className="px-4 xl:px-6 py-3 xl:py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider w-48">
                    Code
                  </th>
                  <th className="px-4 xl:px-6 py-3 xl:py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider w-32">
                    Discount
                  </th>
                  <th className="px-4 xl:px-6 py-3 xl:py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider w-36">
                    Type
                  </th>
                  <th className="px-4 xl:px-6 py-3 xl:py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider min-w-[220px]">
                    Valid Period
                  </th>
                  <th className="px-4 xl:px-6 py-3 xl:py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider w-40">
                    Usage
                  </th>
                  <th className="px-4 xl:px-6 py-3 xl:py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider w-32">
                    Status
                  </th>
                  <th className="px-4 xl:px-6 py-3 xl:py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider w-24">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="bg-gray-900 divide-y divide-gray-800">
                {coupons.map((coupon) => {
                  ...
                  return (
                    <tr key={coupon.id} className={`hover:bg-gray-800 transition-colors ${status === 'expired' ? 'bg-gray-800/50' : ''}`}>
                      <td className="px-4 xl:px-6 py-3 xl:py-4">
                        <div className="flex items-center gap-2">
                          <div className="p-1.5 bg-orange-100 rounded-lg flex-shrink-0">
                            <Ticket className="w-4 xl:w-5 h-4 xl:h-5 text-orange-600" />
                          </div>
                          <div className="min-w-0">
                            <div className="text-sm xl:text-base font-bold font-mono text-white break-words">{coupon.code}</div>
                            {coupon.description && (
                              <div className="text-xs xl:text-sm text-gray-500 mt-0.5 break-words">{coupon.description}</div>
                            )}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          {coupon.discount_type === 'percentage' ? (
                            <Percent className="w-4 xl:w-5 h-4 xl:h-5 text-gray-400" />
                          ) : (
                            <Euro className="w-4 xl:w-5 h-4 xl:h-5 text-gray-400" />
                          )}
                      ...
                      <td className="px-4 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2 xl:px-3 py-1 rounded-full text-xs font-semibold border ${
                            coupon.discount_type === 'percentage'
                              ? 'bg-gradient-to-br from-blue-900 to-cyan-900 text-blue-300 border-blue-800'
                              : 'bg-gradient-to-br from-purple-900 to-indigo-900 text-purple-300 border-purple-800'
                          }`}
                        >
                          <Tag className="w-3 xl:w-3.5 h-3 xl:h-3.5" />
                          {coupon.discount_type === 'percentage' ? 'Percentage' : 'Fixed'}
                        </span>
                      </td>
                      <td className="px-4 xl:px-6 py-3 xl:py-4">
                        <div className="flex items-center gap-2">
                          <Calendar className="w-4 xl:w-5 h-4 xl:h-5 text-gray-400 flex-shrink-0" />
                          <div className="text-sm xl:text-base text-gray-300">
                            <span className="font-medium">{formatDate(coupon.valid_from)}</span>
                            <span className="mx-1 xl:mx-2 text-gray-400">→</span>
                      ...
                      <td className="px-4 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                        {status === 'active' ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2 xl:px-3 py-1 bg-gradient-to-br from-gray-800 to-slate-800 text-gray-300 border border-gray-700 rounded-full text-xs font-semibold">
                            <Clock className="w-3 xl:w-3.5 h-3 xl:h-3.5" />
                      ...
                      <td className={`px-4 xl:px-6 py-3 xl:py-4 whitespace-nowrap ${openMenuId === coupon.id ? 'relative z-50' : ''}`}>
                        <div className="relative" ref={(el) => { menuRefs.current[coupon.id] = el; }}>
                          ...
                          {openMenuId === coupon.id && (
                            <div className="absolute right-0 mt-1 w-48 bg-gray-900 border border-gray-800 rounded-lg shadow-xl z-50 py-1">
                              <button
                                onClick={() => {
                                  handleEdit(coupon);
                                  setOpenMenuId(null);
          ...
          <div className="lg:hidden divide-y divide-gray-800">
            {coupons.map((coupon) => {
              const status = getCouponStatus(coupon);
              return (
                <div key={coupon.id} className={`p-4 ${status === 'expired' ? 'bg-gray-800/50' : ''}`}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <Ticket className="w-4 h-4 text-orange-600" />
                      <div>
                        <h3 className="font-bold font-mono text-white text-lg">{coupon.code}</h3>
                        {coupon.description && (
                    ...
                    {status === 'active' ? (
                      ...
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-800 text-gray-300 border border-gray-800 rounded-full text-xs font-semibold">
                  ...
                  <div className="space-y-2 text-sm mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-white">{formatDiscount(coupon.discount_type, coupon.discount_value)}</span>
                      <Tag className="w-3.5 h-3.5 text-gray-400 ml-2" />
                      <span className="text-gray-400 capitalize">{coupon.discount_type === 'percentage' ? 'Percentage' : 'Fixed'}</span>
                    </div>
                    <div className="flex items-center gap-2">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 overflow-y-auto p-2 sm:p-4">
          ...
          <div className="flex min-h-full items-center justify-center">
            <div
              className="relative bg-gray-900 rounded-2xl shadow-2xl max-w-2xl w-full border border-gray-800 overflow-hidden max-h-[90vh] overflow-y-auto"
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 bg-gray-800 text-white placeholder-gray-400 border border-gray-700 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all font-mono"
                      required
                ...
                {/* Validation Preview */}
                <div className="bg-gradient-to-r from-blue-900 to-indigo-900 border-2 border-blue-800 rounded-xl p-4">
                ...
                <div>
                  <label className="flex items-center space-x-3 cursor-pointer group">
                    <input
                      ...
                      className="w-5 h-5 text-orange-600 border-gray-700 rounded focus:ring-orange-500"
                    />
