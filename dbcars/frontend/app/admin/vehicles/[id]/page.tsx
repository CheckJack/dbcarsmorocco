    ...
    <>
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2">Edit Vehicle</h1>
            <p className="text-gray-400 text-sm sm:text-base md:text-lg">Update vehicle information and settings</p>
          </div>
          <button
            onClick={() => router.back()}
            className="px-4 py-2 bg-gray-900 border border-gray-800 rounded-xl hover:bg-gray-800 transition-all font-semibold flex items-center justify-center gap-2 shadow-sm hover:shadow-md text-gray-300 w-full sm:w-auto"
          >
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        {/* Basic Information */}
        <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-sm p-4 sm:p-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-sm p-4 sm:p-6">
          <div className="flex items-center gap-3 mb-4">
            <h3 className="text-lg sm:text-xl font-bold text-white">Available Extras</h3>
          </div>
          <div className="border-2 border-gray-800 rounded-xl p-3 sm:p-4 max-h-64 overflow-y-auto">
            {extras.length === 0 ? (
              ...
              <div className="space-y-2">
                {extras.map((extra) => (
                  <label
                    key={extra.id}
                    className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 cursor-pointer hover:bg-gray-800 p-3 rounded-xl border border-gray-800 hover:border-orange-300 transition-all group"
                  >
                    <div className="flex items-center gap-3 flex-1">
                      <input
                        ...
                        className="w-5 h-5 text-orange-600 border-gray-700 rounded focus:ring-orange-500 flex-shrink-0"
                      />
                      <span className="text-sm font-medium text-gray-300 flex-1">
                        {extra.name}
                      </span>
                    </div>
                    <span className="text-sm font-bold text-white sm:ml-auto">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-sm p-4 sm:p-6">
          <div className="flex items-center gap-3 mb-4">
            <div>
              <h3 className="text-lg sm:text-xl font-bold text-white">Vehicle Unit Information</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 border border-gray-700 rounded-xl bg-gray-800 text-gray-500 cursor-not-allowed"
                disabled
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-300 mb-2">VIN</label>
              <input
                type="text"
                value={formData.vin}
                className="w-full px-4 py-2.5 border border-gray-700 rounded-xl bg-gray-800 text-gray-500 cursor-not-allowed"
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-300 mb-2">Mileage (km)</label>
              <input
                type="number"
              ...
              <label className="block mb-2">
                ...
                <div className="relative">
                  <input
                    ...
                    disabled={uploading}
                    className="block w-full text-xs sm:text-sm text-gray-500 file:mr-2 sm:file:mr-4 file:py-2 file:px-3 sm:file:px-4 file:rounded-xl file:border-0 file:text-xs sm:file:text-sm file:font-semibold file:bg-gradient-to-r file:from-blue-600 file:to-blue-500 file:text-white hover:file:from-blue-700 hover:file:to-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer"
                  />
                </div>
              </label>
                    ...
                    return (
                      <div
                        ...
                        className={`relative border-2 rounded-xl overflow-hidden shadow-sm transition-all ${
                          isCover ? 'border-blue-500 shadow-md ring-2 ring-blue-200' : 'border-gray-800 hover:border-gray-700'
                        }`}
                      >
                        {isCover && (
                        ...
                        <div className="absolute inset-0 bg-black bg-opacity-0 hover:bg-opacity-30 sm:hover:bg-opacity-30 active:bg-opacity-50 sm:active:bg-opacity-0 transition-all duration-200 flex items-center justify-center group pointer-events-none">
                          <div className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity flex gap-2 pointer-events-auto">
                            {!isCover && (
                              <button
                                type="button"
                        ...
                        <div className="p-2 sm:p-3 bg-gray-900 border-t border-gray-800">
                          <div className="flex items-center justify-between mb-2 gap-2">
                            <p className="text-xs text-gray-500 truncate flex-1 min-w-0" title={imageUrl}>
                          ...
                          <div className="flex gap-1">
                            <button
                              ...
                              disabled={index === 0}
                              className={`flex-1 px-2 py-1.5 sm:py-1 text-xs rounded-lg font-semibold transition-all ${
                                index === 0
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-sm p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <button
              type="submit"
              className="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-500 text-white rounded-xl hover:from-blue-700 hover:to-blue-600 transition-all font-semibold flex items-center justify-center gap-2 shadow-md hover:shadow-lg"
            >
            ...
            <button
              type="button"
              onClick={() => router.back()}
              className="w-full sm:w-auto px-6 py-3 bg-gray-900 border border-gray-800 text-gray-300 rounded-xl hover:bg-gray-800 transition-all font-semibold flex items-center justify-center gap-2 shadow-sm hover:shadow-md"
            >
              <X className="w-5 h-5" />
              Cancel
            </button>
