...
router.get('/:bookingNumber', async (req: Request, res: Response) => {
  try {
    ...
    console.log('[Backend] Booking number length:', bookingNumber.length);
    console.log('[Backend] Booking number chars:', Array.from(bookingNumber).map(c => `${c}(${c.charCodeAt(0)})`).join(' '));
    ...
    if (result.rows.length === 0) {
      console.log('[Backend] Booking not found for number:', bookingNumber);
      const recentBookings = await pool.query('SELECT booking_number FROM bookings ORDER BY created_at DESC LIMIT 5');
      console.log('[Backend] Recent bookings:', recentBookings.rows.map(b => b.booking_number));
      
      // Check if there are similar booking numbers (possible typo)
      const similarBookings = await pool.query(
        `SELECT booking_number FROM bookings 
         WHERE booking_number ILIKE $1 
         ORDER BY created_at DESC LIMIT 3`,
        [`%${bookingNumber.substring(3, 10)}%`]  // Check middle part of booking number
      );
      
      return res.status(404).json({ 
        error: 'Booking not found', 
        booking_number: bookingNumber,
        message: `No booking found with number "${bookingNumber}". Please verify the booking number is correct.`,
        similar_bookings: similarBookings.rows.length > 0 ? similarBookings.rows.map(b => b.booking_number) : undefined
      });
    }
    ...
    console.error('[Backend] Error fetching booking:', error);
