  ...
  try {
    ...
    if (shouldSendEmail) {
      ...
      const emailBookingData = {
        ...
        vehicle_images: fullBooking.images,
      };
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Automatically cancel existing bookings on this date if blocking/maintenance
      if (note_type === 'blocked' || note_type === 'maintenance') {
        ...
        if (vehicle_id) {
          bookingQuery += ` AND vs.vehicle_id = $${paramCount}`;
          bookingParams.push(vehicle_id);
          paramCount++;
        } else if (vehicle_subunit_id) {
          bookingQuery += ` AND vs.id = $${paramCount}`;
          bookingParams.push(vehicle_subunit_id);
          paramCount++;
        }

        const bookingsResult = await pool.query(bookingQuery, bookingParams);
        
        if (bookingsResult.rows.length > 0) {
          // Automatically cancel these bookings
          const bookingIds = bookingsResult.rows.map(b => b.id);
          const cancelNote = `Automatically cancelled due to ${note_type} scheduled on ${noteDate.toLocaleDateString()}`;
          ...
          await pool.query(
            `UPDATE bookings 
             SET status = 'cancelled', 
                 ...
                 updated_at = NOW()
             WHERE id = ANY($2::uuid[])`,
            [cancelNote, bookingIds]
          );

          console.log(`Automatically cancelled ${bookingsResult.rows.length} booking(s) due to ${note_type} note:`, 
            bookingsResult.rows.map(b => b.booking_number)
          );
        }
      }
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Cancel bookings if changing to maintenance/blocked or changing date of maintenance/blocked note
      const finalNoteType = note_type !== undefined ? note_type : existingNote.note_type;
      const finalNoteDate = note_date !== undefined ? new Date(note_date) : new Date(existingNote.note_date);

      if (finalNoteType === 'blocked' || finalNoteType === 'maintenance') {
        const startDate = new Date(finalNoteDate);
        startDate.setHours(0, 0, 0, 0);
        const endDate = new Date(finalNoteDate);
        endDate.setHours(23, 59, 59, 999);
        ...
        if (existingNote.vehicle_id) {
          bookingQuery += ` AND vs.vehicle_id = $${bParamCount}`;
          bookingParams.push(existingNote.vehicle_id);
          bParamCount++;
        } else if (existingNote.vehicle_subunit_id) {
          bookingQuery += ` AND vs.id = $${bParamCount}`;
          bookingParams.push(existingNote.vehicle_subunit_id);
          bParamCount++;
        }
        ...
        if (bookingsResult.rows.length > 0) {
          const bookingIds = bookingsResult.rows.map(b => b.id);
          const cancelNote = `Automatically cancelled due to ${finalNoteType} scheduled on ${finalNoteDate.toLocaleDateString()}`;
          ...
          console.log(`Automatically cancelled ${bookingsResult.rows.length} booking(s) due to ${finalNoteType} note update:`,
            bookingsResult.rows.map(b => b.booking_number)
          );
        }
      }
